...
<div class="min-h-screen text-white p-4 sm:p-6 md:p-8 relative pb-[200px] sm:pb-[160px] md:pb-[120px]">
    ...
    <div class="max-w-6xl mx-auto mb-6">
        <nav class="flex items-center space-x-2 text-sm text-gray-400">
            ...
            <span class="text-gray-300" style="font-family: Inter, 'Helvetica Neue', Helvetica, sans-serif; font-size: 14px;">
                Автор: <?php echo esc_html($author->display_name); ?>
            </span>
        </nav>
    </div>
    ...
    <div class="text-center mb-12">
        <div class="flex justify-center mb-6">
            <div class="w-24 h-24 relative  flex items-center justify-center overflow-hidden">
                <?php if (get_avatar_url($author->ID)) : ?>
                    <?php echo get_avatar($author->ID, 96, '', '', array('class' => ' w-24 h-24 object-cover')); ?>
                <?php else : ?>
                    <img src="<?php echo get_template_directory_uri(); ?>/assets/cooltmp.webp" alt="<?php echo esc_attr($author->display_name); ?>" class="w-24 h-24 object-cover" />
                <?php endif; ?>
            </div>
        </div>
        
        <div class="flex justify-center mb-4">
            <div class="inline-flex items-center px-4 py-2 bg-green-500 bg-opacity-20 ">
                <svg class="w-5 h-5 mr-2 text-green-400" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" />
                </svg>
                <span class="text-green-400 font-medium">Автор</span>
            </div>
        </div>
        ...
        <div class="flex justify-center space-x-6 text-sm text-gray-400">
            <div class="flex items-center">
                ...
                <strong><?php echo $author_posts_count; ?></strong> <?php echo _n('статья', 'статьи', $author_posts_count); ?>
            </div>
            <div class="flex items-center">
                <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" />
                </svg>
                Зарегистрирован: <?php echo get_the_author_meta('user_registered', $author->ID) ? date('F Y', strtotime(get_the_author_meta('user_registered', $author->ID))) : 'Неизвестно'; ?>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-6xl mx-auto">
        <div class="mb-8">
            <h2 class="text-2xl font-bold text-white mb-2" style="font-family: Inter, 'Helvetica Neue', Helvetica, sans-serif;">
                Статьи автора
            </h2>
            <p class="text-gray-400">
                <?php 
                $post_count = $wp_query->found_posts;
                echo 'Найдено <strong>' . $post_count . '</strong> ' . _n('статья', 'статьи', $post_count) . ' от <strong class="text-green-400">' . esc_html($author->display_name) . '</strong>';
                ?>
            </p>
        </div>
            ...
            <?php else : ?>
                <div class="text-center py-12 col-span-full">
                    <div class="mb-6">
                        ...
                        <h3 class="text-xl font-semibold text-gray-400 mb-2">Статьи этого автора не найдены</h3>
                        <p class="text-gray-500">Пока нет публикаций от автора "<?php echo esc_html($author->display_name); ?>"</p>
                    </div>
                    <div class="space-x-4">
                        <a href="<?php echo home_url('/blog'); ?>" class="inline-block px-6 py-3 bg-[#D06E31] text-white font-semibold  hover:bg-[#B85A28] transition-colors">
                            Все статьи
                        </a>
