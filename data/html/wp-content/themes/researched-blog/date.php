<?php
/**
 * Template for displaying date archive pages
 */
...
<div class="min-h-screen text-white p-4 sm:p-6 md:p-8 relative pb-[200px] sm:pb-[160px] md:pb-[120px]">
    ...
    <div class="text-center mb-12">
        <div class="flex justify-center mb-4">
            <div class="inline-flex items-center px-4 py-2 bg-purple-500 bg-opacity-20 ">
                <svg class="w-5 h-5 mr-2 text-purple-400" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" />
                </svg>
                <span class="text-purple-400 font-medium">Архив</span>
            </div>
        </div>
        ...
        <h1 class="mb-4 leading-tight text-white" style="font-size: 36px; sm:font-size: 42px; font-weight: 700; font-family: Inter, 'Helvetica Neue', Helvetica, sans-serif;">
            <?php 
            if (is_year()) {
                echo 'Статьи за ' . get_the_date('Y') . ' год';
            } elseif (is_month()) {
                echo 'Статьи за ' . get_the_date('F Y');
            } elseif (is_day()) {
                echo 'Статьи за ' . get_the_date('j F Y');
            }
            ?>
        </h1>
        ...
        <p class="text-base sm:text-lg text-gray-300 max-w-3xl mx-auto leading-relaxed" style="font-family: Inter, 'Helvetica Neue', Helvetica, sans-serif;">
            ...
            $period = '';
            if (is_year()) {
                $period = 'за ' . get_the_date('Y') . ' год';
            } elseif (is_month()) {
                $period = 'за ' . get_the_date('F Y');
            } elseif (is_day()) {
                $period = 'за ' . get_the_date('j F Y');
            }
            echo 'Найдено <strong>' . $post_count . '</strong> ' . _n('статья', 'статьи', $post_count) . ' <strong class="text-purple-400">' . $period . '</strong>';
            ?>
        </p>
    </div>
            ...
            <?php else : ?>
                <div class="text-center py-12 col-span-full">
                    <div class="mb-6">
                        ...
                        <h3 class="text-xl font-semibold text-gray-400 mb-2">Статьи за этот период не найдены</h3>
                        <p class="text-gray-500">
                            Пока нет публикаций
                            <?php 
                            if (is_year()) {
                                echo 'за ' . get_the_date('Y') . ' год';
                            } elseif (is_month()) {
                                echo 'за ' . get_the_date('F Y');
                            } elseif (is_day()) {
                                echo 'за ' . get_the_date('j F Y');
                            }
                            ?>
                        </p>
                    </div>
