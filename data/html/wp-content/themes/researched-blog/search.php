...
<div class="min-h-screen text-white p-4 sm:p-6 md:p-8 relative pb-[200px] sm:pb-[160px] md:pb-[120px]">
    ...
    <div class="text-center mb-12">
        <h1 class="mb-4 leading-tight text-white" style="font-size: 36px; sm:font-size: 42px; font-weight: 700; font-family: Inter, 'Helvetica Neue', Helvetica, sans-serif;">
            Поиск по блогу researched.xyz
        </h1>
        <?php if (is_year()) : ?>
            <h1 class="text-3xl font-bold text-white mb-6" style="font-family: Inter, 'Helvetica Neue', Helvetica, sans-serif;">
                Поиск: "<?php echo esc_html(get_search_query()); ?>"
            </h1>
        <?php endif; ?>
        <p class="text-base sm:text-xl text-gray-300 leading-relaxed" style="font-family: Inter, 'Helvetica Neue', Helvetica, sans-serif;">
            <?php if (have_posts()) : ?>
                Найдено <strong><?php echo $wp_query->found_posts; ?></strong> <?php echo _n('результат', 'результата', $wp_query->found_posts); ?> по запросу "<strong class="text-[#D06E31]"><?php echo esc_html(get_search_query()); ?></strong>"
            <?php else : ?>
                По запросу "<strong class="text-[#D06E31]"><?php echo esc_html(get_search_query()); ?></strong>" ничего не найдено
            <?php endif; ?>
        </p>
    </div>
        ...
        <div id="posts-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <article class="bg-[#222223] shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer transform hover:scale-105  overflow-hidden" onclick="window.location.href='<?php echo esc_url(get_permalink()); ?>'">
                    ...
                    <div class="p-6">
                        ...
                        <div class="text-gray-300 text-sm leading-relaxed line-clamp-3 mb-4" style="font-family: Inter, 'Helvetica Neue', Helvetica, sans-serif;">
                            <?php
                            // Показываем excerpt с подсветкой поискового запроса
                            ...
                            if ($search_query) {
                                $excerpt = str_ireplace($search_query, '<mark style="background-color: rgba(208, 110, 49, 0.3); padding: 1px 2px; border-radius: 0;">' . $search_query . '</mark>', $excerpt);
                            }
                            echo $excerpt;
                            ?>
                        </div>
            ...
            <?php else : ?>
                <div class="text-center py-12 col-span-full">
                    <h3 class="text-xl font-semibold text-gray-400 mb-4">Ничего не найдено</h3>
                    <p class="text-gray-500 mb-6">Попробуйте изменить запрос или воспользуйтесь другими ключевыми словами</p>
                    
                    <div class="space-y-4">
                        <h4 class="text-lg font-semibold text-gray-300">Рекомендации:</h4>
                        <ul class="text-left text-gray-400 space-y-2 max-w-md mx-auto">
                            <li>• Проверьте правильность написания</li>
                            <li>• Попробуйте более общие ключевые слова</li>
                            <li>• Попробуйте другие синонимы</li>
                            <li>• Используйте меньше слов</li>
                        </ul>
                    </div>
                    
                    <a href="<?php echo home_url('/blog'); ?>" class="inline-block mt-6 px-6 py-3 bg-[#D06E31] text-white font-semibold  hover:bg-[#B85A28] transition-colors">
